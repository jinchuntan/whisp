...
log = logging.getLogger("persephone.router")

# Provider errors that mean "did not really run / consumed no credit" — recorded
# as SKIPPED (excluded from the Agora daily credit count) rather than ERROR.
SKIP_CODES = frozenset(
    {AGORA_NOT_CONFIGURED, AGORA_LIVE_DISABLED, AGORA_SDK_UNAVAILABLE, AGORA_CREDIT_LIMIT_REACHED}
)
    ...
    async def transcribe(self, audio_path: Path, question_id: str) -> RouterOutcome:
        ...
        for index, name in enumerate(self.order):
            ...
            try:
                ...
                # ProviderTimeout -> timeout; skip-codes -> skipped (no credit); else error.
                if isinstance(exc, ProviderTimeout):
                    status = ATTEMPT_TIMEOUT
                elif exc.code in SKIP_CODES:
                    status = ATTEMPT_SKIPPED
                else:
                    status = ATTEMPT_ERROR
                attempts.append(
                    self._record(name, index, status, started, code=exc.code, msg=exc.safe_message)
                )
                last_code, last_msg = exc.code, exc.safe_message
                log.warning(
                    "q=%s provider=%s attempt=%d -> %s(%s)",
                    question_id,
                    name,
                    index,
                    status,
