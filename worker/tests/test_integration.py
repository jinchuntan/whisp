...
deliberately:
    ...
    PERSEPHONE_RUN_INTEGRATION=1 PERSEPHONE_SAMPLE_WAV=/path/to/speech.wav \
    ...
    PERSEPHONE_RUN_INTEGRATION=1 PERSEPHONE_TEST_SUPABASE=1 ... pytest tests/test_integration.py -q
"""
...
RUN = os.environ.get("PERSEPHONE_RUN_INTEGRATION") == "1"

pytestmark = pytest.mark.skipif(
    not RUN, reason="set PERSEPHONE_RUN_INTEGRATION=1 to run manual integration tests"
)


def test_faster_whisper_transcribes_real_wav():
    """Transcribe a real spoken WAV with the real model (no mocks)."""
    sample = os.environ.get("PERSEPHONE_SAMPLE_WAV")
    if not sample or not Path(sample).is_file():
        pytest.skip("set PERSEPHONE_SAMPLE_WAV=/path/to/speech.wav (16 kHz mono PCM16)")
...
def test_supabase_claim_roundtrip():
    ...
    if os.environ.get("PERSEPHONE_TEST_SUPABASE") != "1":
        pytest.skip("set PERSEPHONE_TEST_SUPABASE=1 (uses real Supabase credentials)")
    ...
    q = JobQueue(
        client, os.environ.get("SUPABASE_AUDIO_BUCKET", "persephone-audio"), "itest-worker"
    )
