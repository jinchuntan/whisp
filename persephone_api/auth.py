...
* **Badge** (unchanged): ESP32 badges send ``X-Persephone-Key: <BADGE_API_KEY>``.
...
PRIMARY_BADGE_HEADER = "X-Persephone-Key"
# DEPRECATED compatibility alias, honored for ONE migration period so already-
# deployed badges keep working until they are reflashed. Firmware, docs, and the
# dashboard use the primary header only. Remove this alias (and the second Header
# param below) once all badges send X-Persephone-Key. There is no security
# difference between the two headers.
LEGACY_BADGE_HEADER = "X-Whisp-Key"
BADGE_HEADER = PRIMARY_BADGE_HEADER  # back-compat for importers
ACCESS_COOKIE = "persephone_at"
REFRESH_COOKIE = "persephone_rt"
...
async def require_badge_key(
    x_persephone_key: str | None = Header(default=None, alias=PRIMARY_BADGE_HEADER),
    x_whisp_key: str | None = Header(default=None, alias=LEGACY_BADGE_HEADER),  # DEPRECATED
    settings: Settings = Depends(get_settings),
) -> None:
    provided = _resolve_badge_key(x_persephone_key, x_whisp_key)
    if not _safe_equal(provided, settings.badge_api_key):
        raise HTTPException(
...
def _resolve_badge_key(primary: str | None, legacy: str | None) -> str | None:
    """Accept the primary or the deprecated legacy badge header.

    If BOTH are sent with DIFFERENT values, reject the request (ambiguous). If both
    are sent with the same value, that value is used. The primary header wins.
    """
    if primary is not None and legacy is not None and primary != legacy:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Conflicting badge key headers",
        )
