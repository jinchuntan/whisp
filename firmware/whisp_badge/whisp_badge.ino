// ===========================================================================
// Whisp badge firmware — ESP32-S3 (MakerEdu MKE-K01 N16R8)
//
// Hold the button to record a question; release to upload. The badge uploads a
// standard PCM16 WAV to the Whisp API, receives a question id (HTTP 202), polls
...
#if defined(WHISP_INSECURE_TLS) && WHISP_INSECURE_TLS
  secureClient.setInsecure();  // DEV ONLY — no cert validation.
#else
  if (strlen(WHISP_ROOT_CA) > 0) {
    secureClient.setCACert(WHISP_ROOT_CA);
  } else {
    // No CA pinned and insecure disabled: https will fail (by design).
    Serial.println("[TLS] No root CA and WHISP_INSECURE_TLS off — https will fail.");
  }
#endif
}
...
String uploadRecording() {
  ...
  http.addHeader("X-Whisp-Key", BADGE_API_KEY);
...
void setup() {
  ...
  displayInit();
  showStatus("BOOTING", C_TEAL, "Whisp badge starting...");
