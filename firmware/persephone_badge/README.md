# Persephone Badge Firmware (ESP32-S3)

Arduino sketch for the Persephone lanyard badge. Hold the button to record a
...
3. For `https://` URLs, either paste a root CA into `PERSEPHONE_ROOT_CA`, or (dev
   only) uncomment `#define PERSEPHONE_INSECURE_TLS 1`. Insecure mode disables
...
3. Release → build a 44-byte WAV header → `POST /api/v1/questions`
   (`X-Persephone-Key`, `X-Badge-Id`, `X-Round-Id`).
...
arduino-cli compile --fqbn esp32:esp32:esp32s3 firmware/persephone_badge
```
